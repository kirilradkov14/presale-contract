...
describe('Presale testing', () => { 
 ...
 describe('Presale testing', () => { 
    ...
    it("Pass args, deposit tokens, reach HC, finish sale, claim", async () =>  {
        ...
        const presaleFactory = await ethers.getContractFactory("Presale");
        const presale = await presaleFactory.deploy(token.address, 18, '0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D', '0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f','0xced1cB80C96D4b98DbcBbD20af69A5396Ec3507C', '0xB4FBF271143F4FBf7B91A5ded31805e42b2208d6', '0x9ae56ecb7df61d5d1f90D39781419109a8279753', false, false);
        await presale.deployed();
        console.log('Presale at: '  + presale.address);
        ...
        const timestampNow = Math.floor(new Date().getTime()/1000);
        const initSale = await presale.connect(creator).initSale(timestampNow + 35, timestampNow + 450, timestampNow + 250 ,75, BigInt(70000000000 * (10**18)), BigInt(50000000000*(10**18)), BigInt(3000000000000000), BigInt(2000000000000000), BigInt(3000000000000000), BigInt(3000000000000));
        await initSale.wait();
        console.log('Sale initialized');
